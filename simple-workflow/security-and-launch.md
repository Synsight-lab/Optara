...
| Retroactive fee change | Fees snapshotted at initialization; no setter ever touches them afterward. |
...
| Governance changes a live series | Every series parameter is set once, by `initialize`, and never again. `ADMIN` has no setter that touches a vault. |
| Fake or misleading token metadata | Names are generated by the factory ("Optara WMON/USDC Call #N") from the allowlisted tokens' own symbols. Nothing a user types reaches token metadata. Frontends read details from `seriesInfo()`. |
...
The scripts in `contracts/script/` do these steps. Rehearse the whole thing first with `contracts/script/local/rehearsal.sh` (a local Anvil chain, asserts every result).

1. **`Deploy.s.sol`**: deploys `VaultDeployer` (which deploys the one `OptionSeriesVault` implementation every series will clone), then `OptionSeriesFactory(admin, deployer)` (which binds itself to the deployer), then `PremiumExecutionGuard(factory, ...)`. It sets the default fee rates and the fee recipient and grants `PAUSER`. The broadcaster starts as `ADMIN`. Fee rates must be set **before** the first `createSeries`, because each series snapshots them for ever; a series created against the wrong defaults must be abandoned and recreated.
2. **`ConfigurePair.s.sol`**, once per pair, run by `ADMIN`: allowlists the two assets, approves the pair's feed with `setPairConfig` (feed, age window, strike step) and sets the underlying's open-interest cap. It prints the feed's decimals and latest answer so two reviewers can compare them with Chainlink's feed page **before** the transaction is sent.
3. Set any non-default guard parameters (`setSellerDiscountToleranceBps`, `setMaxReferenceAge`, `setMaxVenueFeeBps`).
4. **`HandOverAdmin.s.sol`**, last: grants `ADMIN` to the multisig or timelock, then renounces the deployer's role. It checks both steps. Afterward the deployer holds nothing.
5. Anyone can now create series, so start with low caps.
...
4. **Give the transaction a generous explicit gas limit** (about 50,000 gas per account is typical; measured around 42,500 on average). `payout` refuses to start an account with under 250,000 gas left and reverts with `InsufficientGas`, so a too-low estimate fails loudly instead of paying only some accounts.
5. Re-read balances afterward. Accounts that were skipped, such as a blacklisted recipient or a contract, stay owed and can redeem themselves.
...
- [ ] Every series parameter and fee rate is set once, by `initialize`, with no setter.
- [ ] The vault implementation can never itself be initialized (`_disableInitializers`), a clone can never be initialized twice, and two clones never share storage - tested directly in `VaultDeployer.t.sol`.
